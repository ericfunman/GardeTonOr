...
class TestOpenAIService:
    ...
    def test_extract_contract_data_telephone(
        ...
        mock_response.choices[0].message.content = json.dumps(
            mock_openai_response_extraction["data"]
        )
    ...
    def test_compare_with_market(
        self,
        mock_openai_class,
        sample_contract_data_telephone,
        mock_openai_response_market
    ):
        ...
        mock_response = Mock()
        mock_response.choices[0].message.content = json.dumps(
            mock_openai_response_market["analysis"]
        )
        mock_client.chat.completions.create.return_value = mock_response
    ...
    def test_compare_with_competitor(
        ...
        mock_response = Mock()
        mock_response.choices[0].message.content = json.dumps(
            mock_openai_response_competitor["analysis"]
        )
        mock_client.chat.completions.create.return_value = mock_response
    ...
    def test_build_extraction_prompt_telephone(self):
        ...
        assert "téléphonie mobile" in prompt.lower()
        assert "fournisseur" in prompt
        assert "prix_mensuel" in prompt
    ...
    def test_build_extraction_prompt_pno(self):
        ...
        assert "assurance" in prompt.lower()
        assert "propriétaire non occupant" in prompt.lower()
        assert "garanties" in prompt

...
def create_contract(data, contract_type):
    ...
    if contract_type == "electricite":
        contract_data = {
            ...
            "prix_kwh": {
                "base": data["electricite"]["tarifs"]["prix_kwh_ttc"]
            },
            ...
            "estimation_conso_annuelle_kwh": data["electricite"]["consommation_estimee_annuelle_kwh"],
...
def display_contract_info(data):
    ...
    print(f"  Abonnement      : {data['electricite']['tarifs']['abonnement_mensuel_ttc_9kVA']:.2f} €/mois TTC")
    print(f"  Prix kWh        : {data['electricite']['tarifs']['prix_kwh_ttc']:.4f} € TTC")
    print(f"  Promotion       : -{data['electricite']['promotion']['remise_kwh_ht_percent']}% pendant {data['electricite']['promotion']['duree_mois']} mois")
    print(f"  Conso estimée   : {data['electricite']['consommation_estimee_annuelle_kwh']:,} kWh/an".replace(',', ' '))
    ...
    print(f"  Promotion       : -{data['gaz']['promotion']['remise_kwh_ht_percent']}% pendant {data['gaz']['promotion']['duree_mois']} mois")
    print(f"  Conso estimée   : {data['gaz']['consommation_estimee_annuelle_kwh']:,} kWh/an".replace(',', ' '))
    ...
    print(f"  💰 TOTAL        : {data['paiements']['mensualite_electricite_ttc'] + data['paiements']['mensualite_gaz_ttc']} €/mois")
    ...
    print("\n" + "="*80)
    print(f"💰 BUDGET TOTAL ANNUEL : {data['electricite']['budget_annuel_estime_ttc'] + data['gaz']['budget_annuel_estime_ttc']} € TTC")
    print(f"💳 MENSUALITÉ TOTALE   : {data['paiements']['mensualite_electricite_ttc'] + data['paiements']['mensualite_gaz_ttc']} € TTC")
    print("="*80 + "\n")

...
def display_assurance_habitation(data):
    ...
    with col2:
        ...
        # Equipements
        equipements = []
        if bien.get("cheminee"): equipements.append("🔥 Cheminée")
        if bien.get("piscine"): equipements.append("🏊 Piscine")
        if bien.get("veranda"): equipements.append("☀️ Véranda")
        if bien.get("dependances"): equipements.append("🏚️ Dépendances")
        if bien.get("systeme_securite"): equipements.append("🚨 Alarme")

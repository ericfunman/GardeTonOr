        ...
        with st.form("validate_contract"):
            if contract_type == "telephone":
                ...
                with col1:
                    ...
                    data_go = st.number_input("Data (Go)", value=float(extracted_data.get("data_go", 0)), min_value=0.0)
                    prix_mensuel = st.number_input("Prix mensuel (€)", value=float(extracted_data.get("prix_mensuel", 0)), min_value=0.0, step=0.01)
                ...
                with col2:
                    ...
                    engagement_mois = st.number_input("Engagement (mois)", value=int(extracted_data.get("engagement_mois", 0)), min_value=0)
                    
                    date_debut = st.date_input(
                        "Date de début",
                        value=datetime.strptime(extracted_data.get("date_debut", datetime.now().strftime("%Y-%m-%d")), "%Y-%m-%d")
                    )
                    date_anniversaire = st.date_input(
                        "Date anniversaire",
                        value=datetime.strptime(extracted_data.get("date_anniversaire", datetime.now().strftime("%Y-%m-%d")), "%Y-%m-%d")
                    )
                
                options = st.text_area("Options incluses", value="\n".join(extracted_data.get("options", [])))
                conditions = st.text_area("Conditions particulières", value=extracted_data.get("conditions_particulieres", ""))
                ...
                with col1:
                    ...
                    surface_m2 = st.number_input("Surface (m²)", value=float(bien.get("surface_m2", 0)), min_value=0.0)
                    nombre_pieces = st.number_input("Nombre de pièces", value=int(bien.get("nombre_pieces", 0)), min_value=0)
                
                with col2:
                    prime_annuelle = st.number_input("Prime annuelle (€)", value=float(extracted_data.get("prime_annuelle", 0)), min_value=0.0, step=0.01)
                    prime_mensuelle = st.number_input("Prime mensuelle (€)", value=float(extracted_data.get("prime_mensuelle", 0)), min_value=0.0, step=0.01)
                    franchise = st.number_input("Franchise (€)", value=float(extracted_data.get("franchise", 0)), min_value=0.0, step=0.01)
                    
                    date_effet = st.date_input(
                        "Date d'effet",
                        value=datetime.strptime(extracted_data.get("date_effet", datetime.now().strftime("%Y-%m-%d")), "%Y-%m-%d")
                    )
                    date_anniversaire = st.date_input(
                        "Date anniversaire",
                        value=datetime.strptime(extracted_data.get("date_anniversaire", datetime.now().strftime("%Y-%m-%d")), "%Y-%m-%d")
                ...
                col1, col2 = st.columns(2)
                with col1:
                    incendie = st.number_input("Incendie (€)", value=float(garanties.get("incendie", 0)), min_value=0.0)
                    degats_eaux = st.number_input("Dégâts des eaux (€)", value=float(garanties.get("degats_des_eaux", 0)), min_value=0.0)
                with col2:
                    vol = st.number_input("Vol (€)", value=float(garanties.get("vol", 0)), min_value=0.0)
                    rc = st.number_input("Responsabilité civile (€)", value=float(garanties.get("responsabilite_civile", 0)), min_value=0.0)
                ...
                try:
                    with get_db() as db:
                        ...
                        contract = contract_service.create_contract(
                            contract_type=st.session_state["contract_type"],
                            provider=provider,
                            start_date=datetime.strptime(validated_data.get("date_debut") or validated_data.get("date_effet"), "%Y-%m-%d"),
                            anniversary_date=datetime.strptime(validated_data["date_anniversaire"], "%Y-%m-%d"),
                        ...
                        # Nettoyer la session
                        for key in ["extracted_data", "pdf_bytes", "filename", "contract_type", "extraction_done"]:

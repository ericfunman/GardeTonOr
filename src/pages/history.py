    ...
    with get_db() as db:
        ...
        for comp in comparisons:
            if comp.comparison_result:
                savings = (
                    comp.comparison_result.get("economie_potentielle_annuelle", 0) or
                    (comp.comparison_result.get("economie_potentielle_mensuelle", 0) * 12) or
                    comp.comparison_result.get("comparaison_prix", {}).get("economie_potentielle", 0)
                )
                if savings > 0:
        ...
        with col1:
            filter_type = st.selectbox(
                "Type d'analyse",
                ["Toutes", "Analyses de marché", "Comparaisons concurrent"]
            )
        
        with col2:
            contracts = contract_service.get_all_contracts()
            contract_options = ["Tous"] + [f"{c.provider} ({CONTRACT_TYPES.get(c.contract_type)})" for c in contracts]
        ...
        if filter_type == "Analyses de marché":
            filtered_comparisons = [c for c in filtered_comparisons if c.comparison_type == "market_analysis"]
        elif filter_type == "Comparaisons concurrent":
            filtered_comparisons = [c for c in filtered_comparisons if c.comparison_type == "competitor_quote"]
        
        if filter_contract != "Tous":
            selected_contract_name = filter_contract.split(" (")[0]
            filtered_comparisons = [
                c for c in filtered_comparisons
                if c.contract.provider == selected_contract_name
            ]
        ...
        for comp in filtered_comparisons:
            if comp.comparison_result:
                ...
                chart_data.append({
                    ...
                    "Type": "Marché" if comp.comparison_type == "market_analysis" else "Concurrent"
                })
